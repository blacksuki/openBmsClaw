...
已有交付物只指向出处、不重建（如技术路线 → 05.technology_roadmap.md ADR + 阶段门；软件架构 → 06.software_architecture.md；API 规范 → 08.api_spec.md + soc_api.h + interface-contract）。标“缺失→建模板”的 4 项模板见 `0_System/templates/`：`feature_list_template.md`、`stakeholder_register_template.md`、`validation_plan_template.md`、`hardware_architecture_template.md`。
