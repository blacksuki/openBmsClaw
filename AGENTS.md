...
### 2.4 参考项目层（已废弃）
原参考项目 `90.mini-Lite/` 已废弃，不再随仓库维护。它曾用于理解多口输出、底座 / 托盘充电逻辑、LED 与按键行为、温度保护和多口功率限制。

这些行为现已直接定义在 `openBmsClaw/services/` 中（`dock/`、`ui/`、`power/`、`bms/`），以当前代码事实为准，不再依赖外部参考项目。
...
### 4.5 产品行为直接在当前工程中定义
原参考项目 `90.mini-Lite` 已废弃。以下行为现以 `openBmsClaw/services/` 为当前代码事实，不再从外部参考项目照搬：
...
芯片选型、目录结构和产品规格一律以 `0_System/*.md` 主线和 `openBmsClaw/` 代码事实为准。
