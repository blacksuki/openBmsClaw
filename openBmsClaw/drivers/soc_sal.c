...
bool soc_sal_init(uint8_t i2c_addr)
{
    ...
    /* 读取芯片 ID 验证通信 */
    uint8_t chip_id = 0u;
    hal_i2c_status_t status = hal_i2c_read(s_soc_i2c_addr, REG_SOC_CHIP_ID, &chip_id, 1u, CONFIG_I2C_TIMEOUT_MS);

    if (status != HAL_I2C_OK)
    {
        if (board_has_uart_log())
        {
            board_uart_write_string("SAL: SoC chip probe failed, offline or locked.\r\n");
        }
        s_sal_initialized = false;
        return false;
    }
...
void soc_sal_bus_recovery(void)
{
    ...
    {
        char msg[80];
        (void)snprintf(msg, sizeof(msg), "SAL: I2C bus lock detected (#%u)! Executing GPIO 9-pulse recovery...\r\n", (unsigned int)s_recovery_count);
        board_uart_write_string(msg);
    }
...
soc_sal_status_t soc_get_voltage(uint32_t *voltage_mv)
{
    ...
    if (voltage_mv == 0) { return SOC_SAL_ERR_PARAM; }
    if (!s_sal_initialized) { return SOC_SAL_ERR_OFFLINE; }

    hal_i2c_status_t status = hal_i2c_read(s_soc_i2c_addr, REG_SOC_BAT_VOLT_H, buf, 2u, CONFIG_I2C_TIMEOUT_MS);
...
soc_sal_status_t soc_get_current(int32_t *current_ma)
{
    ...
    if (current_ma == 0) { return SOC_SAL_ERR_PARAM; }
    if (!s_sal_initialized) { return SOC_SAL_ERR_OFFLINE; }

    hal_i2c_status_t status = hal_i2c_read(s_soc_i2c_addr, REG_SOC_BAT_CURR_H, buf, 2u, CONFIG_I2C_TIMEOUT_MS);
...
soc_sal_status_t soc_get_temperature(int16_t *temp_celsius)
{
    ...
    if (temp_celsius == 0) { return SOC_SAL_ERR_PARAM; }
    if (!s_sal_initialized) { return SOC_SAL_ERR_OFFLINE; }

    hal_i2c_status_t status = hal_i2c_read(s_soc_i2c_addr, REG_SOC_CHIP_TEMP, &raw_temp, 1u, CONFIG_I2C_TIMEOUT_MS);
...
soc_sal_status_t soc_set_ocp(soc_port_t port, uint32_t limit_ma)
{
    ...
    switch (port)
    {
        case SOC_PORT_TYPE_C1: reg = REG_SOC_PORT_C1_OCP; break;
        case SOC_PORT_TYPE_C2: reg = REG_SOC_PORT_C2_OCP; break;
        case SOC_PORT_TYPE_A1: reg = REG_SOC_PORT_A1_OCP; break;
        default: return SOC_SAL_ERR_PARAM;
    }

    /* 简易编码：每 500mA 对应 1 阶 */
    val = (uint8_t)(limit_ma / 500u);

    hal_i2c_status_t status = hal_i2c_write(s_soc_i2c_addr, reg, &val, 1u, CONFIG_I2C_TIMEOUT_MS);
...
{
    /* 假定通过转换公式调整对应输出限额 */
    uint32_t ocp_limit = (uint32_t)max_power_w * 1000u / 5u; /* 假定 5V 下的等效电流 */
...
soc_sal_status_t soc_poll_events(uint32_t *event_mask)
{
    ...
    if (event_mask == 0) { return SOC_SAL_ERR_PARAM; }
    if (!s_sal_initialized) { return SOC_SAL_ERR_OFFLINE; }

    hal_i2c_status_t status = hal_i2c_read(s_soc_i2c_addr, REG_SOC_EVENT_FLAGS, &raw_flags, 1u, CONFIG_I2C_TIMEOUT_MS);
