...
void board_init(void)
{
    ...
    GPIOA_CRH =
        (GPIOA_CRH & ~(UART_TX_CRH_MASK | UART_RX_CRH_MASK)) |
        UART_TX_AF_PP_50MHZ |
        UART_RX_INPUT_FLOATING;
    board_uart_init();
...
{
    return BOARD_HAS_UART_LOG;
}

bool board_has_i2c_bus(void)
{
    return BOARD_HAS_I2C_BUS;
}

bool board_has_adc_channel(void)
{
    return BOARD_HAS_ADC_CHANNEL;
}
